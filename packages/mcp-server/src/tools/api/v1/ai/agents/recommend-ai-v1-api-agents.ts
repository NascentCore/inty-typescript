...
export const tool: Tool = {
  ...
  inputSchema: {
    ...
    properties: {
      ...
      sort: {
        type: 'string',
        title: 'AgentSortOption',
        description: 'Sort order: created_asc, created_desc, random',
        enum: ['created_asc', 'created_desc', 'random'],
      },
      jq_filter: {
        type: 'string',
        title: 'jq Filter',
...
export const handler = async (client: Inty, args: Record<string, unknown> | undefined) => {
  ...
  return asTextContentResult(await maybeFilter(jq_filter, await client.api.v1.ai.agents.recommend(body)));
};
